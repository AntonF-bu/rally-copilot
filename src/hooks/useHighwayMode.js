...
export function useHighwayMode() {
  ...
  const getChatter = useCallback(() => {
    ...
    // BUG FIX #5: Use threshold crossing instead of proximity window
    ...
    for (const item of timeline) {
      ...
      if (overshoot >= 0 && overshoot < MAX_OVERSHOOT) {
        ...
        console.log(`💬 CHATTER CHECK: car at ${Math.round(currentDist)}m, trigger at ${Math.round(triggerDist)}m, overshoot=${Math.round(overshoot)}m`)
