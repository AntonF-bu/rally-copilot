...
export default function RoutePreview({ onStartNavigation, onBack }) {
  ...
  const routeStats = {
    distance: routeData?.distance ? (routeData.distance / 1609.34).toFixed(1) : 0, // miles
    duration: routeData?.duration ? Math.round(routeData.duration / 60) : 0, // minutes
    curves: routeData?.curves?.length || 0,
    sharpCurves: routeData?.curves?.filter(c => c.severity >= 4).length || 0
  }

  // Get curve severity breakdown
  ...
  useEffect(() => {
    ...
    map.current.on('load', () => {
      ...
      startEl.innerHTML = `
        <div style="width: 24px; height: 24px; background: #22c55e; border: 3px solid white; border-radius: 50%; box-shadow: 0 2px 10px rgba(34,197,94,0.5);"></div>
      `
      ...
      endEl.innerHTML = `
        <div style="width: 24px; height: 24px; background: #ef4444; border: 3px solid white; border-radius: 50%; box-shadow: 0 2px 10px rgba(239,68,68,0.5);"></div>
      `
      new mapboxgl.Marker({ element: endEl })
        .setLngLat(routeData.coordinates[routeData.coordinates.length - 1])
        .addTo(map.current)

      // Add curve markers for sharp curves
      if (routeData.curves) {
  ...
  return (
    <div className="fixed inset-0 bg-[#0a0a0f]">
      ...
      <button
        onClick={onBack}
        className="absolute top-4 left-4 z-20 hud-glass w-12 h-12 rounded-xl flex items-center justify-center safe-top"
      >
        ...
        <div className="hud-glass rounded-t-3xl p-4 pb-6">
          ...
          <div className="flex items-center gap-2 mb-5">
            <div className="flex-1 h-2 rounded-full bg-white/10 overflow-hidden flex">
              {severityBreakdown.easy > 0 && (
                <div 
                  className="h-full bg-green-500" 
                  style={{ width: `${(severityBreakdown.easy / routeStats.curves) * 100}%` }}
                />
              )}
              {severityBreakdown.medium > 0 && (
                <div 
                  className="h-full bg-yellow-500" 
                  style={{ width: `${(severityBreakdown.medium / routeStats.curves) * 100}%` }}
                />
              )}
              {severityBreakdown.hard > 0 && (
                <div 
                  className="h-full bg-red-500" 
                  style={{ width: `${(severityBreakdown.hard / routeStats.curves) * 100}%` }}
                />
              )}
            </div>
            <div className="flex items-center gap-3 text-[10px]">
              <span className="flex items-center gap-1">
                ...
                <span className="text-white/40">{severityBreakdown.easy}</span>
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-yellow-500"></span>
                <span className="text-white/40">{severityBreakdown.medium}</span>
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-red-500"></span>
                <span className="text-white/40">{severityBreakdown.hard}</span>
              </span>
            </div>
          </div>
